...
<template>
  <div v-if="user" class="min-h-screen bg-gray-50 font-sans">
    <div class="flex min-h-screen">
      ...
      <!--         SIDEBAR             -->
      <!-- =========================== -->
      <aside class="w-64 flex-none bg-white border-r border-gray-200 flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
          <router-link to="/dashboard" class="flex items-center px-4 py-2.5 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors group">
            <span class="font-medium">Home</span>
          </router-link>
          <router-link to="/dashboard/manage/users" class="flex items-center px-4 py-2.5 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors">
            Manage Users
          </router-link>
          <router-link to="/dashboard/manage/pcBuild" class="flex items-center px-4 py-2.5 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors">
            Manage PC Build
          </router-link>
          <router-link to="#" class="flex items-center px-4 py-2.5 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors">
            Manage Item
          </router-link>

          <!-- Menu Khusus Penjual -->
          <div v-if="user.role === 'penjual'" class="mt-4">
            <p class="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Penjual</p>
            <router-link to="/dashboard/manage/create-toko" class="flex items-center px-4 py-2.5 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors">
              Kelola Toko
            </router-link>
          </div>
          ...
          <div v-if="user.role === 'admin'" class="mt-4">
            ...
            <router-link to="/dashboard/manage/seller-requests" class="flex items-center px-4 py-2.5 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors">
              Request Penjual
            </router-link>
          </div>
        </nav>
        ...
        <div class="p-4 border-t border-gray-200">
          <button @click="logout" class="w-full flex items-center px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            Logout
          </button>
        </div>
      </aside>
      ...
      <main class="flex-1 p-8 overflow-y-auto h-screen">
        ...
        <header class="flex justify-between items-center mb-8">
          <div>
            ...
            <p class="text-gray-500 mt-1">Selamat datang di panel kontrol Anda.</p>
          </div>
          <div class="flex items-center gap-3">
            <span
              ...
              :class="{
                ...
                'bg-yellow-100 text-yellow-800': user.role === 'penjual_pending',
              }"
            >
              {{ user.role.replace("_", " ") }}
            </span>
        ...
        <section class="mb-8">
          ...
          <div v-if="user.role === 'user'" class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
            ...
            <p class="mb-4 opacity-90">Daftarkan akun Anda menjadi penjual untuk mulai membuka toko dan menjual produk rakitan PC.</p>
          ...
          <div v-else-if="user.role === 'penjual_pending'" class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-yellow-700">Permintaan Anda untuk menjadi penjual sedang ditinjau oleh Admin.</p>
            </div>
          </div>
          ...
          <div v-else-if="user.role === 'penjual'" class="bg-green-50 border border-green-200 rounded-xl p-6">
            <div class="flex justify-between items-center">
              <div>
                <h3 class="text-lg font-bold text-green-800">Panel Penjual Aktif</h3>
                <p class="text-green-700">Anda memiliki akses penuh untuk mengelola toko.</p>
              </div>

  ...
  <div class="min-h-screen bg-gray-50 font-ubuntu">
    ...
    <div class="bg-white border-b border-gray-100 pb-8 pt-6">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div class="flex flex-col items-center justify-center">
          <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-pink-50 shadow-sm mb-4">
            <img 
          ...
          <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ user?.store_name || 'Nama Toko' }}</h1>
          <p class="text-gray-500 max-w-2xl mx-auto">{{ user?.description || 'Deskripsi toko belum diatur.' }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="px-6 py-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm text-gray-600">
            <thead class="bg-gray-50/50 text-gray-700 font-semibold uppercase tracking-wider">
              <tr>
            ...
            <tbody class="divide-y divide-gray-50">
              <tr v-if="loading">
                <td colspan="5" class="px-6 py-12 text-center text-gray-500">Memuat produk...</td>
              </tr>
              ...
              >
                <td class="px-6 py-4">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <div class="font-semibold text-gray-900 line-clamp-1 group-hover:text-pink-600 transition-colors">{{ product.nama_produk }}</div>
                      <div class="text-xs text-gray-500 mt-0.5">{{ product.merek }}</div>
                    </div>
                  </div>
                </td>
        ...
        <!-- Pagination -->
        <div class="px-6 py-4 border-t border-gray-100 flex justify-between items-center bg-gray-50/30" v-if="filteredProducts.length > 0">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-2xl shadow-xl max-w-6xl w-full p-6 transform transition-all scale-100 max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-xl font-bold text-gray-900">{{ isEditing ? 'Edit Produk' : 'Tambah Produk Baru' }}</h3>
        ...
        <form @submit.prevent="saveProduct">
          <div class="space-y-6">
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
              <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wider mb-4">Informasi Produk</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                  <label class="block text-sm font-medium text-gray-700 mb-1">Nama Produk</label>
                  <input v-model="form.nama_produk" type="text" required placeholder="Contoh: Laptop Gaming Zenith X1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e84797] focus:border-transparent outline-none transition">
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Merek</label>
                  <input v-model="form.merek" type="text" required placeholder="Contoh: Zenith" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e84797] focus:border-transparent outline-none transition">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                  ...
                  <textarea v-model="form.deskripsi" rows="3" placeholder="Jelaskan detail produkmu..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e84797] focus:border-transparent outline-none transition"></textarea>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="flex justify-between items-center mb-4">
                <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wider">Varian Produk</h4>
                <button type="button" @click="addVariant" class="text-sm text-pink-600 font-medium hover:text-pink-700 flex items-center gap-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                  </svg>
                  Tambah Varian
              ...
              <div class="space-y-4">
                <div v-for="(variant, index) in form.variants" :key="index" class="bg-white border border-gray-200 rounded-xl p-4 relative group hover:border-pink-200 transition-colors">
                  <button v-if="form.variants.length > 1" type="button" @click="removeVariant(index)" class="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                  </button>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                    <!-- Image Upload -->
                    <div class="md:col-span-2">
                      <label class="block text-xs font-medium text-gray-500 mb-1">Foto Varian</label>
                      <div class="relative w-full aspect-square bg-gray-50 rounded-lg border border-dashed border-gray-300 hover:border-pink-400 transition-colors overflow-hidden flex items-center justify-center cursor-pointer group-image">
                        <img v-if="variant.gambar_preview" :src="variant.gambar_preview" class="w-full h-full object-cover">
                        ...
                        <input type="file" @change="(e) => handleFileChange(e, index)" accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer">
                      </div>
                    </div>

                    <!-- Variant Details -->
                    <div class="md:col-span-10 grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Nama Varian</label>
                        <input v-model="variant.nama_varian" type="text" placeholder="Contoh: Merah, XL, 256GB" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e84797] focus:border-transparent outline-none text-sm">
                      </div>
                      <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Harga (Rp)</label>
                        <input v-model="variant.harga" type="number" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e84797] focus:border-transparent outline-none text-sm">
                      </div>
                      <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Stok</label>
                        <input v-model="variant.stok" type="number" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e84797] focus:border-transparent outline-none text-sm">
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="mt-8 flex gap-3 pt-6 border-t border-gray-100">
            <button type="button" @click="closeModal" class="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 transition">
              Batal
            </button>
            <button 
              type="submit" 
    ...
    <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-2xl shadow-xl max-w-3xl w-full p-6 transform transition-all scale-100 max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-start mb-6">
          <div>
            <h3 class="text-xl font-bold text-gray-900">{{ selectedProduct?.nama_produk }}</h3>
        ...
        <div class="space-y-6">
          <!-- Description -->
          <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
            <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wider mb-2">Deskripsi</h4>
            <p class="text-gray-600 whitespace-pre-line">{{ selectedProduct?.deskripsi || 'Tidak ada deskripsi.' }}</p>
          </div>

          <!-- Variants -->
          <div>
            <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wider mb-3">Varian Produk</h4>
            <div class="overflow-hidden border border-gray-200 rounded-xl">
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Foto</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Varian</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="variant in selectedProduct?.variant" :key="variant.id_varian">
                    <td class="px-4 py-3 whitespace-nowrap">
                      <div class="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
                        <img v-if="variant.gambar_varian" :src="`http://127.0.0.1:8000/storage/${variant.gambar_varian}`" class="w-full h-full object-cover">
        ...
        <div class="mt-8 flex justify-end">
          <button @click="closeDetailModal" class="px-6 py-2 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 transition">
            Tutup
          </button>
        </div>
      </div>
...
const openEditModal = (product) => {
  ...
  const categoryDetailId = product.category_detail && product.category_detail.length > 0 ? product.category_detail[0].id : null;
  ...
  const variants = (product.variant || []).map(v => ({
    ...
    gambar_preview: v.gambar_varian ? `http://127.0.0.1:8000/storage/${v.gambar_varian}` : null
  }));
...
const saveProduct = async () => {
  if (!form.value.nama_produk || !form.value.kategori) {
  ...
  try {
    ...
    // Handle Category
    if (isEditing.value && form.value.category_detail_id) {
        formData.append('detail[0][id]', form.value.category_detail_id);
        formData.append('detail[0][kategori]', form.value.kategori);
    } else {
        formData.append('kategori[]', form.value.kategori);
        // Also for update if detail id is missing for some reason
        if (isEditing.value) formData.append('detail[0][kategori]', form.value.kategori);
    }

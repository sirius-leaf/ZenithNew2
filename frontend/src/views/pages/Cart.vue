...
const initializeChecklist = (summary) => {
    ...
    summary.forEach(item => {
        ...
        initialChecks[item.variant.id_varian] = checkedItems.value[item.variant.id_varian] !== undefined 
...
const filteredTotalPrice = computed(() => {
    return filteredCartForCheckout.value.reduce((total, item) => total + item.subtotal, 0);
});
...
const goToCheckout = () => {
    ...
    const selectedItemsForCheckout = filteredCartForCheckout.value.map(item => ({
        id_varian: item.variant.id_varian,
        kuantitas: item.kuantitas
    }));
    
    // Ini adalah 'handshake' antara Cart dan Checkout
    localStorage.setItem('checkout_selection', JSON.stringify(selectedItemsForCheckout));
                ...
                <div v-for="item in cartSummary" :key="item.variant.id_varian"
                    ...
                    <div class="w-24 flex flex-col items-center">
                        ...
                        <input 
                            ...
                            @change="updateKuantitas(item.variant.id_varian, $event.target.valueAsNumber)"
                            min="1"
            ...
            <div class="lg:col-span-1">
                <div class="bg-white p-6 rounded-lg shadow-lg border border-gray-100 sticky top-4">
                    ...
                    <button @click="goToCheckout"
                        :disabled="filteredCartForCheckout.length === 0 || apiError || loading"

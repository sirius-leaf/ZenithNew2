...
const { cartItems, removeCartItem, updateCartItem, totalItems } =
  useCartStore();
...
const initializeChecklist = (summary) => {
  ...
  summary.forEach((item) => {
    ...
    initialChecks[item.variant.id_varian] =
      checkedItems.value[item.variant.id_varian] !== undefined
        ? checkedItems.value[item.variant.id_varian]
        : true;
  });
...
const updateKuantitas = (id_varian, kuantitas) => {
  ...
  updateCartItem(
    id_varian,
    kuantitas - cartItems.value.find((i) => i.id_varian === id_varian).kuantitas
  );
  fetchCartPreview();
};
...
const removeItem = (id_varian) => {
  if (confirm("Yakin ingin menghapus item ini?")) {
    ...
    // Hapus dari checklist juga
    delete checkedItems.value[id_varian];
    fetchCartPreview();
  }
};

// Fungsi pindah ke checkout - HANYA MENGIRIM ITEM YANG DICEKLIS
const goToCheckout = () => {
  ...
  // SIMPAN SEMENTARA ITEM YANG DIPILIH KE LOCAL STORAGE
  const selectedItemsForCheckout = filteredCartForCheckout.value.map(
    (item) => ({
  ...
  // Ini adalah 'handshake' antara Cart dan Checkout
  localStorage.setItem(
    "checkout_selection",
    JSON.stringify(selectedItemsForCheckout)
  );
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="flex-1 min-w-0 pr-4">
            <h3 class="text-lg font-semibold text-gray-900 truncate">
              ...
              <small class="text-gray-500"
                >({{ item.variant.nama_varian }})</small
              >
            </h3>
            <p class="text-sm text-gray-600">
              Toko: {{ item.variant.product.toko.toko_name }}
            </p>
            <p class="text-md font-medium text-green-600 mt-1">
          ...
          <div class="w-24 flex flex-col items-center">
            ...
            <input
              ...
              "
              min="1"
              class="w-full text-center border border-gray-300 rounded-md py-1 text-sm focus:ring-blue-500"
            />
          </div>
      ...
      <div class="lg:col-span-1">
        ...
        >
          ...
          <div
            class="flex justify-between text-lg font-semibold mt-4 pt-3 border-t border-gray-200"
          >
            <span>Total Harga ({{ totalCheckedItems }} Item Dipilih)</span>
            <span class="text-green-600"
              >Rp {{ filteredTotalPrice.toLocaleString("id-ID") }}</span
            >
          </div>
          ...
          <button
            @click="goToCheckout"
            :disabled="
              filteredCartForCheckout.length === 0 || apiError || loading
            "
            class="w-full mt-6 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-lg transition disabled:bg-gray-400"
          >
            Lanjut ke Checkout
          </button>

...
<template>
  <div
    class="bg-white rounded-xl shadow-md p-6 animate-fade-in max-w-5xl mx-auto"
  >
      ...
      >
        ...
        <div class="flex justify-between items-start mb-4">
          <div>
            <h3 class="font-semibold text-blue-900">
              {{ seller.store_name || seller.storeName }}
            </h3>
        ...
        <div class="mt-4 pt-4 border-t border-gray-200">
          <p class="text-xs text-gray-500 italic">
            ⚠️ Upload file (KTP/NPWP) belum diimplementasi — akan ditambahkan di
            versi berikutnya.
          </p>
        </div>
      </div>
...
const fetchSellerRequests = async () => {
  ...
  try {
    ...
    sellers.value = res.data.data.map((s) => ({
      ...s,
      processing: false,
    }));
  } catch (error) {

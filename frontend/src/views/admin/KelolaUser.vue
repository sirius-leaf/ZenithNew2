...
<template>
  <div class="bg-white rounded-xl shadow-md p-4 md:p-6 animate-fade-in max-w-4xl mx-auto mt-8">
    ...
    <div v-if="loading" class="py-10 text-center">
      <div class="inline-block w-8 h-8 border-4 border-pink-300 border-t-pink-600 rounded-full animate-spin"></div>
    ...
    <div v-else class="overflow-x-auto rounded-lg border border-gray-200">
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          >
            ...
            <td class="py-3 px-3 text-blue-700 truncate max-w-xs">{{ user.email }}</td>
            <td class="py-3 px-3">
              <span
      ...
      <!-- Empty -->
      <div v-if="users.length === 0" class="py-10 px-4 text-center text-gray-500 bg-gray-50 rounded-b-lg">
    ...
    <!-- Pagination -->
    <div v-if="!loading && meta.last_page > 1" class="flex justify-center mt-6 space-x-1">
      <button
        @click="goToPage(1)"
        :disabled="meta.current_page === 1"
        class="px-2 py-1 text-xs font-medium rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
        :class="meta.current_page === 1 ? 'bg-gray-200 text-gray-500' : 'bg-pink-100 text-pink-700 hover:bg-pink-200'"
      >
        &laquo;
      </button>
      ...
      <template v-for="page in visiblePages" :key="page">
        <button
          ...
          :class="meta.current_page === page ? 'bg-pink-500 text-white' : 'bg-pink-100 text-pink-700 hover:bg-pink-200'"
        >
          {{ page }}
        </button>
      ...
      <button
        ...
        class="px-2 py-1 text-xs font-medium rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
        :class="meta.current_page === meta.last_page ? 'bg-gray-200 text-gray-500' : 'bg-pink-100 text-pink-700 hover:bg-pink-200'"
      >
        &raquo;
      </button>
    </div>
...
// 🔒 Ban / Unban
const confirmBan = (id) => {
  if (confirm('⚠️ Yakin ingin membatasi akun ini?\nUser yang terbatasi tidak dapat login.')) {
    banUser(id)
  }
}

...
<div v-if="user" class="max-w-4xl mx-auto mt-8 px-6 sm:px-0">
  ...
  <div v-else class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 text-center">
    ...
    <!-- Modal Incomplete Profile -->
    <Transition name="modal-fade">
      <div v-if="showIncompleteProfileModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click="showIncompleteProfileModal = false">
        <Transition name="modal-slide">
          <div class="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden" @click.stop>
            <div class="p-6 text-center">
              ...
              <p class="text-gray-600 mb-6">
                Untuk mendaftar sebagai penjual, Anda wajib melengkapi <strong>Nomor HP</strong> dan <strong>Alamat</strong> terlebih dahulu.
              </p>
              <div class="flex gap-3 justify-center">
                <button @click="showIncompleteProfileModal = false" class="px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50">
                  Nanti Saja
                </button>
                <button @click="$router.push('/profile/edit')" class="px-4 py-2 bg-[#203f9a] text-white font-medium rounded-lg hover:bg-[#94c2da]">
                  Lengkapi Profil
                </button>
              </div>
            </div>

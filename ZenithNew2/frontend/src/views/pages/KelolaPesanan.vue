  ...
  <div class="font-ubuntu p-6 max-w-7xl mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      ...
      <div class="flex items-center gap-2">
        ...
        <button @click="applySearch" class="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-lg">Cari</button>
      </div>
    </div>
    ...
    <div v-else-if="error" class="text-center py-12 text-pink-600">Gagal memuat data pesanan.</div>
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="order in filteredOrders" :key="order.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-gray-800">Rp {{ parseFloat(order.total_harga).toLocaleString("id-ID") }}</td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3">
              <span class="px-2 py-1 text-xs font-medium rounded-full" :class="getStatusClass(order.status)">
            ...
            <td class="px-4 py-3">
              ...
              <div class="flex flex-wrap gap-1">
                ...
                <div v-else-if="order.status === 'cancellation_requested'" class="flex flex-wrap gap-1">
                  <button
                ...
                <span
                  v-else-if="['shipped', 'completed', 'cancelled'].includes(order.status)"
    ...
    <div v-if="showModal" class="fixed inset-0 backdrop-blur-sm bg-black/20 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl w-full max-w-3xl max-h-[80vh] overflow-y-auto" @click.stop>
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p class="font-medium text-gray-900">ORD-{{ String(selectedOrder?.id).padStart(4, "0") }}</p>
            </div>
            <div>
          ...
          <div
            v-if="selectedOrder?.status === 'cancelled' && selectedOrder?.alasan_pembatalan"
          ...
          <div class="border-t border-gray-100 pt-4">
            <h3 class="font-bold text-gray-800 mb-3 text-sm">Informasi Pembeli</h3>
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p class="text-gray-500">ID User</p>
                <p class="font-medium text-gray-900">#{{ selectedOrder?.user_id }}</p>
              </div>
              <div>
                <p class="text-gray-500">Nama User</p>
                <p class="font-medium text-gray-900">
                  {{ selectedOrder?.user?.name || selectedOrder?.buyer_name || "-" }}
                </p>
              </div>
            ...
            <div class="bg-gray-50 rounded-lg p-3 space-y-3">
              <div
                ...
                <div class="flex-1">
                  ...
                  <p class="text-gray-500 text-xs">Varian: {{ item.variant?.nama_varian }}</p>
          ...
          <div class="border-t border-gray-100 pt-4 flex justify-between items-center">
            <div>
              ...
              <p class="font-bold text-gray-900">
                {{ selectedOrder?.detail_pesanans?.reduce((sum, item) => sum + item.kuantitas, 0) }}
                Pcs
              </p>
            </div>
          ...
          <div v-if="selectedOrder?.resi" class="bg-blue-50 p-3 rounded-lg border border-blue-100 mt-2">
        ...
        <div class="p-5 border-t border-gray-200 text-right">
          <button @click="closeModal" class="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-lg">
            Tutup
          </button>
        </div>
      </div>
    ...
    <div v-if="showResiModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl w-full max-w-md" @click.stop>
        <div class="p-5 border-b border-gray-200">
          <h2 class="text-lg font-bold text-blue-800">Kirim Pesanan</h2>
          <p class="text-sm text-gray-600 mt-1">Pesanan: ORD-{{ String(selectedOrderForResi?.id).padStart(4, "0") }}</p>
        </div>

        <div class="p-5 space-y-4">
          <div>
            <label class="block text-gray-700 text-sm font-medium mb-2">Pilih Ekspedisi / Kurir</label>
            <select
              v-model="selectedCourier"
              @change="onCourierChange"
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-300 bg-white">
              <option value="" disabled>-- Pilih Kurir --</option>
              <option v-for="courier in courierOptions" :key="courier.label" :value="courier.value">
                {{ courier.label }}
              </option>
            </select>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <input
                ...
                :placeholder="
                  selectedCourier
                    ? courierOptions.find((c) => c.value === selectedCourier)?.placeholder
                    : 'Pilih kurir terlebih dahulu atau ketik manual'
                "
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-300 font-mono tracking-wide"
            ...
            <p class="text-xs text-gray-500 mt-2">
              *Pastikan nomor resi sesuai dengan yang tertera pada label pengiriman.
            </p>
          </div>
        </div>

        <div class="p-5 border-t border-gray-200 flex justify-end gap-2">
          <button @click="showResiModal = false" class="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg">
            Batal
          </button>
          <button
...
const selectedCourier = ref(""); // Menyimpan kurir yang dipilih

const courierOptions = [
  { label: "J&T Express", value: "JP", placeholder: "Contoh: JP1234567890" },
  { label: "JNE", value: "JNE", placeholder: "Contoh: 0112233445566 (Biasanya hanya angka)" },
  { label: "SiCepat", value: "00", placeholder: "Contoh: 001234567890" },
  { label: "Shopee Xpress", value: "SPXID", placeholder: "Contoh: SPXID02938475" },
  { label: "AnterAja", value: "1000", placeholder: "Contoh: 10001234567890" },
  { label: "ID Express", value: "IDE", placeholder: "Contoh: IDE123456789" },
  { label: "Ninja Xpress", value: "NLID", placeholder: "Contoh: NLID123456789" },
  { label: "Pos Indonesia", value: "P", placeholder: "Contoh: P123456789" },
  { label: "Lainnya / Manual", value: "", placeholder: "Masukkan nomor resi lengkap" },
];

// Fungsi saat dropdown kurir berubah
const onCourierChange = () => {
  // Reset input ke awalan kurir yang dipilih
  resiInput.value = selectedCourier.value;
};

// ... (kode loadOrders, dll tetap sama)

// Update fungsi openResiModal untuk mereset pilihan kurir
const openResiModal = (order) => {
  selectedOrderForResi.value = order;

  // Jika sudah ada resi sebelumnya, tampilkan
  if (order.resi) {
    resiInput.value = order.resi;
    selectedCourier.value = ""; // Kosongkan kurir jika mode edit
  } else {
    resiInput.value = "";
    selectedCourier.value = ""; // Default kosong
  }
...
const approveCancellation = async (order) => {
  ...
  try {
    await axios.post(`http://127.0.0.1:8000/api/orders/${order.id}/approve-cancellation`);
...
const rejectCancellation = async (order) => {
  ...
  try {
    await axios.post(`http://127.0.0.1:8000/api/orders/${order.id}/reject-cancellation`);
